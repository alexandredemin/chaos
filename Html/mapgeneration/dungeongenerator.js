...
class DungeonGenerator {
    constructor(cfg = {}) {
        ...
        this.loopRatio = cfg.loopRatio ?? .15;
        this.zoneAttempts = Math.max(1, cfg.zoneAttempts || 40);
        this.minRoomSize = cfg.minRoomSize || 4;
        this.maxRoomSize = cfg.maxRoomSize || 14;
        this.roomAreaTarget = cfg.roomAreaTarget || 68;
    ...
    _resetGenerationState() {
        this.rooms = [];
        this.alcoves = [];
        this._alcoveCarved = new Set();
        this.zones = [];
        this.edges = [];
        this.gates = [];
        this._zoneById = new Map();
        this._zoneAtGrid = null;
        this._globalRoomAt = null;
    ...
    _createEmptyMap() { const kind = [], walls = []; for (let y = 0; y < this.height; y++) {
        kind[y] = [];
        walls[y] = [];
    ...
    _applyLayout(layout) {
        if (!(layout instanceof ZoneLayout))
            layout = new ZoneLayout(layout);
        if (layout.width !== this.width || layout.height !== this.height)
            throw new Error('ZoneLayout size does not match DungeonGenerator size');
    ...
    _zone(id) {
        return this._zoneById.get(id) || this.zones.find(z => z.id === id);
    }
    _sharedBoundary(a, b) { return RectGeometry.sharedBoundary(a, b); }
    _safeGateRange(a, b, s) { return RectGeometry.safeGateRange(a, b, s); }
    _addGateRecord(edge, a, b, s, p, optional = false) { let ac, bc; if (s.orientation === 'vertical') {
        if (s.aSide === 'E') {
    ...
    } const g = { id: 'g' + this.gates.length, edgeId: edge.id, a: a.id, b: b.id, aCell: ac, bCell: bc, aSide: s.aSide, bSide: s.bSide, optional }; this.gates.push(g); edge.gates.push(g); if (!edge.gate)
        edge.gate = g; a.gates.push({ gate: g, cell: ac, side: s.aSide, other: b.id, optional }); b.gates.push({ gate: g, cell: bc, side: s.bSide, other: a.id, optional }); return g; }
    _assignGates() { this.gates = []; for (const z of this.zones)
        z.gates = []; for (const edge of this.edges) {
        edge.gates = [];
        edge.gate = null;
        const a = this._zone(edge.a), b = this._zone(edge.b), s = this._sharedBoundary(a, b), range = s && this._safeGateRange(a, b, s);
        if (!s || !range)
            throw new Error('ZoneGraph edge has no safe gate: ' + edge.a + ' <-> ' + edge.b);
    ...
    _splitLeaves(work, target, minSize = this.minRoomSize) { const leaves = [{ ...work }]; while (leaves.length < target) {
        const candidates = [];
    ...
    _roomTarget(zone) { let target = Math.min(this.maxRoomsPerZone, Math.max(1, Math.round(zone.rect.w * zone.rect.h / this.roomAreaTarget))); if (zone.type === 'normal') {
        ...
        if (work.w >= 3 && work.h >= 3) {
            ...
            if (canSplit && long / Math.max(1, short) >= 1.65)
                target = Math.max(target, 2);
        }
    } return target; }
    _rectNear(a, b, pad = 0) { return a.x - pad < b.x + b.w && a.x + a.w + pad > b.x && a.y - pad < b.y + b.h && a.y + a.h + pad > b.y; }
    _generateBspRooms(zone, targetOverride = null, reserved = []) { const singleWork = this._inset(zone.rect, 1); if (singleWork.w < 3 || singleWork.h < 3)
        return []; const target = targetOverride ?? this._roomTarget(zone); if (target === 1)
        return [{ id: 'r' + (this._roomSeq++), zoneId: zone.id, roomType: 'normal', x: singleWork.x, y: singleWork.y, w: singleWork.w, h: singleWork.h, doorOutside: new Set() }]; const work = { ...zone.rect }, minSize = Math.max(3, Math.min(this.minRoomSize, work.w, work.h)), leaves = this._splitLeaves(work, target, minSize), rooms = []; for (const leaf of leaves) {
        const ml = leaf.x === zone.rect.x ? 1 : 0, mr = leaf.x + leaf.w === zone.rect.x + zone.rect.w ? 1 : 0, mt = leaf.y === zone.rect.y ? 1 : 0, mb = leaf.y + leaf.h === zone.rect.y + zone.rect.h ? 1 : 0, usable = { x: leaf.x + ml, y: leaf.y + mt, w: leaf.w - ml - mr, h: leaf.h - mt - mb };
        if (usable.w < 3 || usable.h < 3)
            continue;
        ...
        for (let attempt = 0; attempt < 12 && !placed; attempt++) {
            const w = this._rand(leafMin, maxW), h = this._rand(leafMin, maxH), x = this._rand(usable.x, usable.x + usable.w - w), y = this._rand(usable.y, usable.y + usable.h - h), candidate = { x, y, w, h };
            if (reserved.some(rr => this._rectNear(candidate, rr, 1)))
                continue;
    ...
    _buildRoomAt(rooms) { const roomAt = Array.from({ length: this.height }, () => Array(this.width).fill(null)); for (const r of rooms)
        for (let y = r.y; y < r.y + r.h; y++)
            for (let x = r.x; x < r.x + r.w; x++)
                roomAt[y][x] = r; return roomAt; }
    _gateKeys(zone) { return new Set((zone.gates || []).map(g => this._key(g.cell.x, g.cell.y))); }
    _roomPorts(room, zone, roomAt, gateKeys) { const p = [], add = (ix, iy, ox, oy, side) => { if (!this._insideRect(zone.rect, ox, oy) || roomAt[oy][ox])
        return; if (this._isBoundary(zone.rect, ox, oy) && !gateKeys.has(this._key(ox, oy)) && !zone.allowBoundaryRouting)
        return; p.push({ ix, iy, ox, oy, side }); }; for (let x = room.x; x < room.x + room.w; x++) {
    ...
    _cellAllowed(zone, x, y, roomAt, roomA, roomB, portA, portB, gateKeys) { if (!this._insideRect(zone.rect, x, y) || roomAt[y][x])
        return false; const k = this._key(x, y); if (this._isBoundary(zone.rect, x, y) && !gateKeys.has(k) && !(zone.allowBoundaryRouting && this._isOuterBoundary(x, y)))
        return false; for (let dy = -1; dy <= 1; dy++)
    ...
    _buildLPath(x1, y1, x2, y2, hFirst) { const p = []; if (hFirst) {
        const sx = x1 <= x2 ? 1 : -1;
        for (let x = x1;; x += sx) {
            p.push([x, y1]);
            if (x === x2)
    ...
    _validatePath(path, zone, roomAt, roomA, roomB, portA, portB, gateKeys) { for (const [x, y] of path)
        if (!this._cellAllowed(zone, x, y, roomAt, roomA, roomB, portA, portB, gateKeys))
            return false; return true; }
    _corridorAdjPenalty(x, y, floor) { let side = 0, diag = 0; for (const [dx, dy] of DIR4)
        if (floor.has(this._key(x + dx, y + dy)))
            side++; for (const [dx, dy] of [[1, 1], [1, -1], [-1, 1], [-1, -1]])
        if (floor.has(this._key(x + dx, y + dy)))
            diag++; return side * 4 + diag * .5; }
    _loopCrowdingOK(path, floor) { const add = new Set(path.map(([x, y]) => this._key(x, y)).filter(k => !floor.has(k))); if (!add.size)
        return false; let run = 0, maxRun = 0; const blockOrigins = new Set(); for (let i = 0; i < path.length; i++) {
            ...
            for (const oy of [-1, 0]) {
                ...
                for (let dy = 0; dy < 2; dy++)
                    for (let dx = 0; dx < 2; dx++) {
                        ...
                        if (!(floor.has(kk) || add.has(kk)))
                            full = false;
                        if (add.has(kk))
                            newCount++;
                    }
                if (full && newCount)
                    blockOrigins.add((x + ox) + ',' + (y + oy));
            }
    } return maxRun <= 2 && blockOrigins.size <= 2; }
    _astar(zone, sx, sy, tx, ty, roomAt, roomA, roomB, portA, portB, gateKeys, floor, reuseCost = .55, preferNew = false) { const key = (x, y) => this._key(x, y), total = this.width * this.height, g = new Float64Array(total), prev = new Int32Array(total), closed = new Uint8Array(total), heap = new MinHeap(); g.fill(Infinity); prev.fill(-1); const start = key(sx, sy), target = key(tx, ty); if (start !== target && !this._cellAllowed(zone, sx, sy, roomAt, roomA, roomB, portA, portB, gateKeys))
        return null; g[start] = 0; heap.push({ x: sx, y: sy, f: Math.abs(sx - tx) + Math.abs(sy - ty) }); while (heap.length) {
        ...
        for (const [dx, dy] of DIR4) {
            ...
            if (closed[ni] || !this._cellAllowed(zone, nx, ny, roomAt, roomA, roomB, portA, portB, gateKeys))
                continue;
            let step = floor.has(ni) ? reuseCost : 1;
            if (preferNew && !floor.has(ni))
                step += this._corridorAdjPenalty(nx, ny, floor);
            const ng = g[ci] + step;
            if (ng >= g[ni])
                continue;
            g[ni] = ng;
    ...
    _tryPath(zone, sx, sy, tx, ty, roomAt, roomA, roomB, portA, portB, gateKeys, floor, opts = {}) { const minNew = opts.minNewCells || 0, reuseCost = opts.preferNew ? 3 : .55, acceptable = p => { if (!p)
        return false; let n = 0; for (const [x, y] of p)
        if (!floor.has(this._key(x, y)))
            n++; if (n < minNew)
        return false; if (opts.preferNew && !this._loopCrowdingOK(p, floor))
        return false; return true; }; for (const hFirst of [this.random() < .5, true, false]) {
    ...
    } const p = this._astar(zone, sx, sy, tx, ty, roomAt, roomA, roomB, portA, portB, gateKeys, floor, reuseCost, !!opts.preferNew); if (acceptable(p)) {
        this.stats.astar++;
        return p;
    } this.stats.routeFailed++; return null; }
    _carveRoute(path, floor) { for (const [x, y] of path)
        floor.add(this._key(x, y)); }
    _routeRooms(zone, a, b, roomAt, gateKeys, floor, opts = {}) { const pa = this._roomPorts(a, zone, roomAt, gateKeys), pb = this._roomPorts(b, zone, roomAt, gateKeys), pairs = []; for (const x of pa)
        for (const y of pb)
            pairs.push({ x, y, d: Math.abs(x.ox - y.ox) + Math.abs(x.oy - y.oy) }); pairs.sort((u, v) => u.d - v.d); if (opts.preferNew) {
    ...
    _routeGateToRoom(zone, gateCell, room, roomAt, gateKeys, floor) { const ports = this._roomPorts(room, zone, roomAt, gateKeys).sort((a, b) => (Math.abs(a.ox - gateCell.x) + Math.abs(a.oy - gateCell.y)) - (Math.abs(b.ox - gateCell.x) + Math.abs(b.oy - gateCell.y))); for (const p of ports) {
    ...
    _connectRoomTree(zone, rooms, roomAt, gateKeys, floor, edgeSet = new Set()) { if (!rooms.length)
        return false; const comp = new Map(rooms.map((r, i) => [r, i])), find = r => { let x = comp.get(r); while (comp.get(rooms[x]) !== x)
        x = comp.get(rooms[x]); return x; }, merge = (a, b) => { const ra = find(a), rb = find(b); if (ra === rb)
        return; for (const r of rooms)
        if (find(r) === rb)
            comp.set(r, ra); }; while (true) {
    ...
    _corridorLoopAnchors(zone, roomAt, gateKeys, floor) { const a = []; for (const k of floor) {
        const [x, y] = this._xy(k);
        if (roomAt[y][x])
            continue;
        if (this._isBoundary(zone.rect, x, y) && !gateKeys.has(k) && !zone.allowBoundaryRouting)
            continue;
        let near = false;
    ...
    _addCorridorShortcut(zone, roomAt, gateKeys, floor) { const anchors = this._corridorLoopAnchors(zone, roomAt, gateKeys, floor); if (anchors.length < 2)
        return false; const pairs = []; for (let i = 0; i < anchors.length; i++)
        for (let j = i + 1; j < anchors.length; j++) {
            const a = anchors[i], b = anchors[j], d = Math.abs(a.x - b.x) + Math.abs(a.y - b.y);
            if (d >= 5)
                pairs.push({ a, b, d });
        } if (!pairs.length)
        return false; this._shuffle(pairs); pairs.sort((a, b) => b.d - a.d); for (const q of pairs.slice(0, 96)) {
    ...
    _floorDistance(zone, floor, a, b) { const sk = this._key(a.x, a.y), tk = this._key(b.x, b.y); if (sk === tk)
        return 0; if (!floor.has(sk) || !floor.has(tk))
        return Infinity; const seen = new Map([[sk, 0]]), q = [sk]; let qi = 0; while (qi < q.length) {
        const k = q[qi++], d = seen.get(k), [x, y] = this._xy(k);
        for (const [dx, dy] of DIR4) {
            const nx = x + dx, ny = y + dy;
            if (!this._insideRect(zone.rect, nx, ny))
                continue;
            const nk = this._key(nx, ny);
            if (seen.has(nk) || !floor.has(nk))
                continue;
            if (nk === tk)
                return d + 1;
            seen.set(nk, d + 1);
    ...
    _localLoopCandidates(zone, rooms, floor) { const c = []; for (let i = 0; i < rooms.length; i++)
        for (let j = i + 1; j < rooms.length; j++) {
            const a = rooms[i], b = rooms[j], ca = this._center(a), cb = this._center(b), network = this._floorDistance(zone, floor, ca, cb), direct = Math.abs(ca.x - cb.x) + Math.abs(ca.y - cb.y), benefit = isFinite(network) ? Math.max(0, network - direct) : direct, jitter = this.random();
            c.push({ a, b, benefit, direct, jitter });
        } c.sort((x, y) => (y.benefit - x.benefit) || (y.direct - x.direct) || (x.jitter - y.jitter)); return c; }
    _addLocalLoops(zone, rooms, roomAt, gateKeys, floor) { if (this.loopRatio <= 0 || rooms.length < 2)
        return 0; const candidates = this._localLoopCandidates(zone, rooms, floor); this.stats.loopCandidates += candidates.length; const selected = candidates.filter(() => this.random() < this.loopRatio), target = selected.length; this.stats.loopsRequested += target; if (!target)
        return 0; let added = 0; for (const p of selected) {
        if (this._routeRooms(zone, p.a, p.b, roomAt, gateKeys, floor, { preferNew: true, minNewCells: 1 })) {
            added++;
            this.stats.loopsAdded++;
        }
    } let shortcutTries = 0; while (added < target && shortcutTries < Math.max(8, target * 8)) {
        shortcutTries++;
        if (!this._addCorridorShortcut(zone, roomAt, gateKeys, floor))
            continue;
        added++;
        this.stats.loopsAdded++;
    } return added; }
    _optionalGateCandidates(edge) { const a = this._zone(edge.a), b = this._zone(edge.b); if (!a || !b || a.type === 'special' || b.type === 'special')
        return []; const s = this._sharedBoundary(a, b), range = s && this._safeGateRange(a, b, s); if (!s || !range)
        return []; const used = (edge.gates || []).map(g => s.orientation === 'vertical' ? g.aCell.y : g.aCell.x), span = range.hi - range.lo + 1, maxExtra = Math.min(3, Math.floor((span - 1) / 5)); if (maxExtra <= 0)
        return []; const out = [], taken = used.slice(); for (let n = 0; n < maxExtra; n++) {
        let best = null;
    ...
    _snapshotDoors(zones) { const m = new Map(); for (const z of zones)
        for (const r of z.rooms || [])
            m.set(r, new Set(r.doorOutside || [])); return m; }
    _restoreDoors(snapshot) { for (const [r, s] of snapshot)
        r.doorOutside = new Set(s); }
    _connectNewGateSide(zone, cell, floor) { const rooms = zone.rooms || [], roomAt = this._buildRoomAt(rooms), gateKeys = this._gateKeys(zone); gateKeys.add(this._key(cell.x, cell.y)); const anchors = this._corridorLoopAnchors(zone, roomAt, gateKeys, floor).filter(a => a.x !== cell.x || a.y !== cell.y).sort((a, b) => (Math.abs(a.x - cell.x) + Math.abs(a.y - cell.y)) - (Math.abs(b.x - cell.x) + Math.abs(b.y - cell.y))); for (const t of anchors.slice(0, 96)) {
        const path = this._tryPath(zone, cell.x, cell.y, t.x, t.y, roomAt, null, null, null, null, gateKeys, floor);
        if (!path)
            continue;
        this._carveRoute(path, floor);
        return true;
    } const sorted = rooms.slice().sort((a, b) => { const ca = this._center(a), cb = this._center(b); return (Math.abs(ca.x - cell.x) + Math.abs(ca.y - cell.y)) - (Math.abs(cb.x - cell.x) + Math.abs(cb.y - cell.y)); }); for (const r of sorted)
        if (this._routeGateToRoom(zone, cell, r, roomAt, gateKeys, floor))
            return true; return false; }
    _tryAddOptionalGate(edge, candidate) { const a = this._zone(edge.a), b = this._zone(edge.b), s = this._sharedBoundary(a, b); if (!a || !b || !s || a.type === 'special' || b.type === 'special')
        return false; let ac, bc, p = candidate.p; if (s.orientation === 'vertical') {
        if (s.aSide === 'E') {
            ac = { x: a.rect.x + a.rect.w - 1, y: p };
            bc = { x: b.rect.x, y: p };
        }
    ...
    } const fa = new Set(a.floor || []), fb = new Set(b.floor || []), snapshot = this._snapshotDoors([a, b]); fa.add(this._key(ac.x, ac.y)); fb.add(this._key(bc.x, bc.y)); if (!this._connectNewGateSide(a, ac, fa) || !this._connectNewGateSide(b, bc, fb)) {
        this._restoreDoors(snapshot);
        return false;
    } a.floor = fa; b.floor = fb; this._addGateRecord(edge, a, b, s, p, true); this.stats.extraGates++; return true; }
    _addOptionalGateLoops() { if (this.loopRatio <= 0)
        return 0; let added = 0; for (const edge of this.edges) {
        ...
        for (const c of selected)
            if (this._tryAddOptionalGate(edge, c))
                added++;
    } return added; }
    _applyPostGenerationLoops() { if (this.loopRatio <= 0)
        return; this._useLoopRng = true; try {
    ...
    _alcoveRectCells(rect) { const out = []; for (let y = rect.y; y < rect.y + rect.h; y++)
        for (let x = rect.x; x < rect.x + rect.w; x++)
            out.push(this._key(x, y)); return out; }
    _alcoveDoorways(rect) { const p = []; for (let x = rect.x; x < rect.x + rect.w; x++) {
    ...
    _alcoveGlobalFloor() { const floor = new Set(); for (const z of this.zones)
        for (const k of z.floor || [])
            floor.add(k); return floor; }
    _alcoveZoneAt() {
        if (this._zoneAtGrid)
            return this._zoneAtGrid;
    ...
    _alcoveRectRock(rect, floor) { if (!this._alcoveRectInsideMap(rect))
        return false; for (let y = rect.y; y < rect.y + rect.h; y++)
        for (let x = rect.x; x < rect.x + rect.w; x++)
            if (floor.has(this._key(x, y)))
                return false; return true; }
    _alcoveNearRect(rect, x, y) { return x >= rect.x - 1 && x <= rect.x + rect.w && y >= rect.y - 1 && y <= rect.y + rect.h; }
    _alcoveRectHaloClear(rect, floor) { for (let y = rect.y - 1; y <= rect.y + rect.h; y++)
    ...
    _alcoveFloorNeighbors(x, y, floor) { const orth = [], all = []; for (let dy = -1; dy <= 1; dy++)
        for (let dx = -1; dx <= 1; dx++) {
            if (!dx && !dy)
                continue;
            const nx = x + dx, ny = y + dy;
    ...
    _alcoveDirectCandidate(rect, door, floor, roomAt) { const ax = door.ox, ay = door.oy; if (ax <= 0 || ay <= 0 || ax >= this.width - 1 || ay >= this.height - 1 || !floor.has(this._key(ax, ay)))
        return null; let contacts = []; for (let y = rect.y; y < rect.y + rect.h; y++)
        for (let x = rect.x; x < rect.x + rect.w; x++)
            for (const [dx, dy] of DIR4) {
                const nx = x + dx, ny = y + dy, k = this._key(nx, ny);
                if (floor.has(k))
                    contacts.push({ x, y, nx, ny, k });
            } const uniq = new Map(contacts.map(c => [c.x + ',' + c.y + '>' + c.k, c])); contacts = [...uniq.values()]; if (contacts.length !== 1)
        return null; const c = contacts[0]; if (c.x !== door.ix || c.y !== door.iy || c.nx !== ax || c.ny !== ay)
        return null; const anchorRoom = roomAt[ay]?.[ax] || null; if (anchorRoom?.special)
        return null; for (let y = rect.y - 1; y <= rect.y + rect.h; y++)
        for (let x = rect.x - 1; x <= rect.x + rect.w; x++) {
            if (x >= rect.x && x < rect.x + rect.w && y >= rect.y && y < rect.y + rect.h)
                continue;
            const k = this._key(x, y);
            if (!floor.has(k) || k === c.k)
                continue;
            return null;
        } return { door, path: [], attachment: { x: ax, y: ay, k: c.k }, anchorRoom, tunnelLength: 0 }; }
    _alcoveRockRoute(rect, door, floor, roomAt, maxTunnel) { if (maxTunnel < 1)
        return null; const sx = door.ox, sy = door.oy; if (sx <= 0 || sy <= 0 || sx >= this.width - 1 || sy >= this.height - 1 || floor.has(this._key(sx, sy)))
        return null; const start = this._key(sx, sy), q = [start], depth = new Map([[start, 1]]), prev = new Map(), seen = new Set([start]); let qi = 0; while (qi < q.length) {
        ...
        if (d >= maxTunnel || n.all.length)
            continue;
        for (const [dx, dy] of DIR4) {
            const nx = x + dx, ny = y + dy, nk = this._key(nx, ny);
            if (seen.has(nk) || nx <= 0 || ny <= 0 || nx >= this.width - 1 || ny >= this.height - 1 || floor.has(nk))
                continue;
            if (this._alcoveNearRect(rect, nx, ny) && nk !== start)
                continue;
            seen.add(nk);
            depth.set(nk, d + 1);
            prev.set(nk, k);
    ...
    _findAlcoveConnection(rect, floor, maxTunnel, roomAt) { const doors = this._alcoveDoorways(rect), found = []; this._alcoveShuffle(doors); for (const d of doors) {
        ...
        return null; found.sort((a, b) => a.tunnelLength - b.tunnelLength); const top = found.slice(0, Math.min(found.length, 8)); return top[this._alcoveRand(0, top.length - 1)]; }
    _alcoveTouchedZones(rect, path, zoneAt) { const ids = new Set(); for (const k of this._alcoveRectCells(rect)) {
    ...
    _findAlcoveCandidates(kind, minSize, maxSize, maxTunnel) { const out = [], floor = this._alcoveFloor || this._alcoveGlobalFloor(), roomAt = this._globalRoomAt || this._buildRoomAt(this.rooms), zoneAt = this._alcoveZoneAt(); for (let h = minSize; h <= maxSize; h++)
        for (let w = minSize; w <= maxSize; w++) {
            if (w >= this.width - 2 || h >= this.height - 2)
                continue;
            for (let y = 1; y + h < this.height - 1; y++)
                for (let x = 1; x + w < this.width - 1; x++) {
                    const rect = { x, y, w, h };
                    if (!this._alcoveRectRock(rect, floor))
                        continue;
                    const conn = this._findAlcoveConnection(rect, floor, maxTunnel, roomAt);
                    if (!conn || this._alcoveCarved.has(conn.attachment.k))
                        continue;
                    ...
                    if (!zonesTouched.length && (!this.allowUnzonedAlcoves || !attachmentZone))
                        continue;
                    let gateDist = 99;
    ...
    _carveAlcove(c) { const zoneAt = this._alcoveZoneAt(), carved = new Set(), touched = new Set(); const addCell = (x, y) => { const k = this._key(x, y), z = zoneAt[y]?.[x]; if (!z && !this.allowUnzonedAlcoves) throw new Error('Alcove carve outside zone coverage at ' + x + ',' + y); if (z) { z.floor.add(k); touched.add(z.id); } carved.add(k); this._alcoveCarved.add(k); if (this._alcoveFloor) this._alcoveFloor.add(k); }; for (let y = c.rect.y; y < c.rect.y + c.rect.h; y++)
        for (let x = c.rect.x; x < c.rect.x + c.rect.w; x++)
            addCell(x, y); for (const [x, y] of c.path)
        addCell(x, y); const doorOutsideKey = c.path.length ? this._key(c.path[c.path.length - 1][0], c.path[c.path.length - 1][1]) : this._key(c.door.ix, c.door.iy); if (c.anchorRoom)
        c.anchorRoom.doorOutside.add(doorOutsideKey); const attachmentZone = zoneAt[c.attachment.y]?.[c.attachment.x]?.id || null, a = { id: 'alc' + this.alcoves.length, type: c.kind, zoneIds: [...touched], anchorZoneId: attachmentZone, rect: { ...c.rect }, door: { x: c.door.ix, y: c.door.iy }, doorOutside: { x: c.door.ox, y: c.door.oy }, tunnel: c.path.map(([x, y]) => ({ x, y })), attachment: { x: c.attachment.x, y: c.attachment.y }, tunnelLength: c.tunnelLength, crossZone: touched.size > 1 || (attachmentZone && !touched.has(attachmentZone)), touchesSpecial: [...touched].some(id => this._zone(id)?.type === 'special'), carvedKeys: carved }; this.alcoves.push(a); return a; }
    _generateAlcoveKind(kind, count, minSize, maxSize, maxTunnel) { let added = 0; for (let i = 0; i < count; i++) {
        const candidates = this._findAlcoveCandidates(kind, minSize, maxSize, maxTunnel);
        if (kind === 'room')
            this.stats.alcoveRoomCandidates += candidates.length;
        else
    ...
    _validateAlcoves() { const allFloor = this._alcoveFloor || this._alcoveGlobalFloor(), roomAt = this._globalRoomAt || this._buildRoomAt(this.rooms); for (const a of this.alcoves) {
        const carved = a.carvedKeys, outside = new Set();
        for (const k of carved) {
            const [x, y] = this._xy(k);
            if (x <= 0 || y <= 0 || x >= this.width - 1 || y >= this.height - 1 || !allFloor.has(k) || roomAt[y]?.[x])
                return false;
            for (const [dx, dy] of DIR4) {
                const nk = this._key(x + dx, y + dy);
                if (allFloor.has(nk) && !carved.has(nk))
                    outside.add(nk);
            }
        }
        if (outside.size !== 1)
    ...
    _paintAlcoves(map) { for (const a of this.alcoves) {
        for (let y = a.rect.y; y < a.rect.y + a.rect.h; y++)
            for (let x = a.rect.x; x < a.rect.x + a.rect.w; x++)
                map.kind[y][x] = ' ';
        const c = this._center(a.rect);
        map.kind[c.y][c.x] = a.type === 'room' ? 'a' : 'n';
    } }
    _paintFloors(map) { for (const z of this.zones)
        for (const k of z.floor || []) {
            const [x, y] = this._xy(k);
            map.walls[y][x] = null;
            if (map.kind[y][x] === '#')
    ...
    _recordFallback(zone, level) { this.stats.fallbacks++; this.stats.fallbackDetails.push(zone.id + ':' + level); zone.fallback = level; }
    _generateNormalFallback(zone) { const gateKeys = this._gateKeys(zone); for (let level = 1; level <= 3; level++) {
        if (level <= 2) {
            ...
            if (rooms.length) {
                ...
                for (const g of zone.gates || [])
                    floor.add(this._key(g.cell.x, g.cell.y));
                let ok = this._connectRoomTree(zone, rooms, roomAt, gateKeys, floor, edgeSet);
                if (ok)
                    for (const g of zone.gates || []) {
                        let linked = false;
                        for (const r of rooms.slice().sort((a, b) => { const ca = this._center(a), cb = this._center(b); return Math.abs(ca.x - g.cell.x) + Math.abs(ca.y - g.cell.y) - Math.abs(cb.x - g.cell.x) - Math.abs(cb.y - g.cell.y); })) {
    ...
    _generateNormalZone(zone) { const baseTarget = zone.type === 'arena' ? 1 : this._roomTarget(zone), work = this._inset(zone.rect, 1); if (zone.type !== 'arena' && (work.w < 3 || work.h < 3)) {
        ...
        if (attempt)
            this.stats.zoneRetries++;
        let target = baseTarget;
        if (zone.type !== 'arena') {
            const q = (attempt + 1) / this.zoneAttempts;
            if (q > .9)
                target = Math.max(1, baseTarget - 2);
            else if (q > .75)
        ...
        for (const g of zone.gates || [])
            floor.add(this._key(g.cell.x, g.cell.y));
        if (!this._connectRoomTree(zone, rooms, roomAt, gateKeys, floor, edgeSet))
            continue;
        let gatesOK = true;
        for (const g of zone.gates || []) {
            const sorted = rooms.slice().sort((a, b) => { const ca = this._center(a), cb = this._center(b); return (Math.abs(ca.x - g.cell.x) + Math.abs(ca.y - g.cell.y)) - (Math.abs(cb.x - g.cell.x) + Math.abs(cb.y - g.cell.y)); });
            let ok = false;
        ...
        if (!gatesOK)
            continue;
        if (!this._validateLocalZoneGeometry(zone, rooms, floor))
            continue;
        return { rooms, floor };
    } if (zone.type !== 'arena') {
    ...
    _makeArenaRoom(zone) { const r = this._inset(zone.rect, 1); if (r.w < 3 || r.h < 3)
        throw new Error('Arena too small'); return { id: 'r' + (this._roomSeq++), zoneId: zone.id, roomType: 'arena', x: r.x, y: r.y, w: r.w, h: r.h, doorOutside: new Set() }; }
    _routeGateBackbone(zone, roomAt, gateKeys, floor) { const gs = zone.gates || []; if (gs.length < 2)
        return true; for (let i = 1; i < gs.length; i++) {
        const a = gs[0].cell, b = gs[i].cell, path = this._tryPath(zone, a.x, a.y, b.x, b.y, roomAt, null, null, null, null, gateKeys, floor);
        if (!path)
            return false;
        this._carveRoute(path, floor);
    } return true; }
    _routeRoomToFloor(zone, room, roomAt, gateKeys, floor) { const ports = this._roomPorts(room, zone, roomAt, gateKeys), targets = []; for (const k of floor) {
        const [x, y] = this._xy(k);
        if (roomAt[y][x])
            continue;
        let near = false;
        ...
        if (!near)
            targets.push({ x, y });
    } const pairs = []; for (const p of ports)
        for (const t of targets)
            pairs.push({ p, t, d: Math.abs(p.ox - t.x) + Math.abs(p.oy - t.y) }); pairs.sort((a, b) => a.d - b.d); for (const q of pairs.slice(0, 160)) {
    ...
    _generateCompactSpecialZone(zone) { const r = zone.rect, corner = zone.corner, floor = new Set(), east = corner === 'nw' || corner === 'sw', south = corner === 'nw' || corner === 'ne', backX = east ? r.x + r.w - 1 : r.x, backY = south ? r.y + r.h - 1 : r.y; for (let y = r.y; y < r.y + r.h; y++)
        floor.add(this._key(backX, y)); for (let x = r.x; x < r.x + r.w; x++)
        floor.add(this._key(x, backY)); for (const g of zone.gates || [])
        floor.add(this._key(g.cell.x, g.cell.y)); const maxW = Math.min(this.specialMaxRoomSize, r.w - 2), maxH = Math.min(this.specialMaxRoomSize, r.h - 2), minW = Math.min(this.specialMinRoomSize, maxW), minH = Math.min(this.specialMinRoomSize, maxH); if (maxW < 3 || maxH < 3)
        throw new Error('Special zone ' + zone.id + ' too small'); const w = this._rand(Math.max(3, minW), Math.max(3, maxW)), h = this._rand(Math.max(3, minH), Math.max(3, maxH)), x = east ? r.x : r.x + r.w - w, y = south ? r.y : r.y + r.h - h, room = { id: 'r' + (this._roomSeq++), zoneId: zone.id, roomType: zone.roomType, special: true, x, y, w, h, doorOutside: new Set() }; for (let yy = y; yy < y + h; yy++)
        for (let xx = x; xx < x + w; xx++)
            floor.add(this._key(xx, yy)); let door, branch = []; if (r.w - w >= 2) {
    ...
    } for (const [xx, yy] of branch)
        floor.add(this._key(xx, yy)); if (branch.length)
        room.doorOutside.add(this._key(branch[0][0], branch[0][1])); room.specialDoor = door; return { rooms: [room], floor }; }
    _generateStructuredMiddleSpecialFallback(zone) { const r = zone.rect, gateKeys = this._gateKeys(zone), floor = new Set(), rooms = [], isW = zone.slot === 'w', isE = zone.slot === 'e', isN = zone.slot === 'n', isS = zone.slot === 's'; if (!(isW || isE || isN || isS))
        return null; for (const g of zone.gates || [])
        floor.add(this._key(g.cell.x, g.cell.y)); let room, lane, door, doorOut; if (isW || isE) {
        if (r.w < 7 || r.h < 7)
            return null;
        ...
        for (const g of zone.gates || []) {
            ...
            if (g.side === 'N' || g.side === 'S') {
                ...
                for (let x = c.x; x !== lane; x += step)
                    floor.add(this._key(x, iy));
                floor.add(this._key(lane, iy));
            }
            else {
                const step = c.x <= lane ? 1 : -1;
                for (let x = c.x; x !== lane; x += step)
                    floor.add(this._key(x, c.y));
        ...
        for (const g of zone.gates || []) {
            ...
            if (g.side === 'W' || g.side === 'E') {
                ...
                for (let y = c.y; y !== lane; y += step)
                    floor.add(this._key(ix, y));
                floor.add(this._key(ix, lane));
            }
            else {
                const step = c.y <= lane ? 1 : -1;
                for (let y = c.y; y !== lane; y += step)
                    floor.add(this._key(c.x, y));
        ...
        for (let x = room.x; x < room.x + room.w; x++)
            floor.add(this._key(x, y)); room.doorOutside.add(this._key(doorOut.x, doorOut.y)); room.specialDoor = door; rooms.push(room); if (!this._validateLocalZoneGeometry(zone, rooms, floor) || !this._zoneTerminalsConnected(zone, rooms, floor, true))
        return null; this._recordFallback(zone, 'structured-' + zone.slot); return { rooms, floor }; }
    _generateSpecialFallback(zone) { if (zone.slotClass === 'corner') {
    ...
    } zone.allowBoundaryRouting = false; return this._generateStructuredMiddleSpecialFallback(zone); }
    _placeSpecialRoomInLeaf(zone, leaf, small = false) { const narrowW = zone.slotClass === 'middle' && leaf.w <= 5, narrowH = zone.slotClass === 'middle' && leaf.h <= 5, minW = Math.max(3, Math.min(small ? 3 : (narrowW ? 3 : this.specialMinRoomSize), leaf.w)), minH = Math.max(3, Math.min(small ? 3 : (narrowH ? 3 : this.specialMinRoomSize), leaf.h)), maxW = Math.max(minW, Math.min(this.specialMaxRoomSize, leaf.w)), maxH = Math.max(minH, Math.min(this.specialMaxRoomSize, leaf.h)); for (let attempt = 0; attempt < 36; attempt++) {
        ...
        if (zone.slot === 'e' || zone.slot === 'w')
            y = this._rand(leaf.y, leaf.y + leaf.h - h);
        if (zone.slot === 'n' || zone.slot === 's')
            x = this._rand(leaf.x, leaf.x + leaf.w - w);
    ...
    _generateNormalRoomInLeaf(zone, leaf) { const maxW = Math.min(this.maxRoomSize, leaf.w), maxH = Math.min(this.maxRoomSize, leaf.h), minSize = Math.max(3, Math.min(this.minRoomSize, maxW, maxH)); if (maxW < 3 || maxH < 3)
        return null; const w = this._rand(minSize, maxW), h = this._rand(minSize, maxH), x = this._rand(leaf.x, leaf.x + leaf.w - w), y = this._rand(leaf.y, leaf.y + leaf.h - h); return { id: 'r' + (this._roomSeq++), zoneId: zone.id, roomType: 'normal', x, y, w, h, doorOutside: new Set() }; }
    _generateSpecialRoomsFromWholeBsp(zone, target, small = false) { const work = this._inset(zone.rect, 1); if (work.w < 3 || work.h < 3)
        return null; const minSize = Math.max(3, Math.min(this.minRoomSize, work.w, work.h)), leaves = target <= 1 ? [{ ...work }] : this._splitLeaves(work, target, minSize); if (!leaves.length)
        return null; const choices = leaves.map((leaf, i) => ({ leaf, i, area: leaf.w * leaf.h })); this._shuffle(choices); for (const pick of choices) {
        const rooms = [];
    ...
    _generateSpecialZone(zone) { if (zone.slotClass === 'corner' && Math.min(zone.rect.w, zone.rect.h) <= 6) {
        zone.allowBoundaryRouting = true;
        return this._generateCompactSpecialZone(zone);
    } zone.allowBoundaryRouting = false; const baseTarget = this._roomTarget(zone); for (let attempt = 0; attempt < this.zoneAttempts; attempt++) {
        if (attempt)
            this.stats.zoneRetries++;
        ...
        let networkOK = true;
        if (normals.length) {
            if (!this._connectRoomTree(zone, normals, roomAt, gateKeys, floor, edgeSet))
                networkOK = false;
            if (networkOK)
                for (const g of zone.gates || []) {
                    const sorted = normals.slice().sort((a, b) => { const ca = this._center(a), cb = this._center(b); return (Math.abs(ca.x - g.cell.x) + Math.abs(ca.y - g.cell.y)) - (Math.abs(cb.x - g.cell.x) + Math.abs(cb.y - g.cell.y)); });
                    let ok = false;
                    for (const r of sorted)
                        if (this._routeGateToRoom(zone, g.cell, r, roomAt, gateKeys, floor)) {
                            ok = true;
        ...
        if (!networkOK)
            continue;
        if (!this._routeRoomToFloor(zone, special, roomAt, gateKeys, floor))
            continue;
        if (!this._validateLocalZoneGeometry(zone, rooms, floor))
            continue;
        if (!this._zoneTerminalsConnected(zone, rooms, floor, true))
            continue;
        return { rooms, floor };
    } const fb = this._generateSpecialFallback(zone); if (fb)
        return fb; throw new Error('Failed to generate special zone ' + zone.id + ' after ' + this.zoneAttempts + ' attempts and fallback'); }
    _validateLocalZoneGeometry(zone, rooms, floor) { const roomAt = this._buildRoomAt(rooms), gateKeys = this._gateKeys(zone); for (const k of floor) {
        const [x, y] = this._xy(k);
        if (!this._insideRect(zone.rect, x, y))
            return false;
        if (roomAt[y][x])
            continue;
        if (this._isBoundary(zone.rect, x, y) && !gateKeys.has(k) && !(zone.allowBoundaryRouting && this._isOuterBoundary(x, y)))
            return false;
        ...
        for (const [room, t] of touched) {
            if (!room.doorOutside.has(k))
                return false;
            if (!t.orth)
                return false;
        }
    } return this._zoneTerminalsConnected(zone, rooms, floor, false); }
    _zoneTerminalsConnected(zone, rooms, floor, excludeSpecial) { const blocked = new Set(); if (excludeSpecial)
        for (const r of rooms.filter(x => x.special))
            for (let y = r.y; y < r.y + r.h; y++)
                for (let x = r.x; x < r.x + r.w; x++)
                    blocked.add(this._key(x, y)); const terms = []; for (const g of zone.gates || [])
        terms.push(this._key(g.cell.x, g.cell.y)); for (const r of rooms)
        if (!excludeSpecial || !r.special) {
            const c = this._center(r);
            terms.push(this._key(c.x, c.y));
        } if (terms.length <= 1)
        return true; const start = terms[0]; if (!floor.has(start) || blocked.has(start))
        return false; const seen = new Set([start]), q = [start]; let qi = 0; while (qi < q.length) {
        const k = q[qi++], [x, y] = this._xy(k);
        for (const [dx, dy] of DIR4) {
            const nx = x + dx, ny = y + dy, nk = this._key(nx, ny);
            if (!this._insideRect(zone.rect, nx, ny) || seen.has(nk) || blocked.has(nk) || !floor.has(nk))
                continue;
            seen.add(nk);
            q.push(nk);
        }
    } return terms.every(k => seen.has(k)); }
    _mergeZoneResult(zone, res, map) { zone.rooms = res.rooms; zone.floor = res.floor; for (const k of res.floor) {
        const [x, y] = this._xy(k);
        map.walls[y][x] = null;
        if (map.kind[y][x] === '#')
            map.kind[y][x] = '.';
    ...
    _markGatesAndDoors(map) { for (const g of this.gates) {
        map.kind[g.aCell.y][g.aCell.x] = ',';
        map.kind[g.bCell.y][g.bCell.x] = ',';
    } for (const r of this.rooms.filter(x => x.special && x.specialDoor))
        map.kind[r.specialDoor.y][r.specialDoor.x] = 'D'; for (const r of this.rooms) {
        let ch = null;
        if (r.roomType === 'arena')
            ch = 'A';
    ...
    _zoneGraphConnected() { if (!this.zones.length)
        return true; const adj = new Map(this.zones.map(z => [z.id, []])); for (const e of this.edges) {
        adj.get(e.a).push(e.b);
        adj.get(e.b).push(e.a);
    } const seen = new Set([this.zones[0].id]), q = [this.zones[0].id]; let qi = 0; while (qi < q.length) {
        const z = q[qi++];
    ...
    _globalReachableWithoutSpecial() { const floor = new Set(); for (const z of this.zones)
        for (const k of z.floor || [])
            floor.add(k); const blocked = new Set(); for (const r of this.rooms.filter(x => x.special))
        for (let y = r.y; y < r.y + r.h; y++)
            for (let x = r.x; x < r.x + r.w; x++)
                blocked.add(this._key(x, y)); const normals = this.rooms.filter(x => !x.special); if (!normals.length)
        return true; const c = this._center(normals[0]), start = this._key(c.x, c.y), seen = new Set([start]), q = [start]; let qi = 0; while (qi < q.length) {
        const k = q[qi++], [x, y] = this._xy(k);
        for (const [dx, dy] of DIR4) {
            const nx = x + dx, ny = y + dy;
            if (nx < 0 || ny < 0 || nx >= this.width || ny >= this.height)
                continue;
            const nk = this._key(nx, ny);
            if (seen.has(nk) || blocked.has(nk) || !floor.has(nk))
                continue;
            seen.add(nk);
            q.push(nk);
        }
    } return normals.every(r => { const c = this._center(r); return seen.has(this._key(c.x, c.y)); }); }
    _specialEntranceCount(room) { let n = 0; const floor = new Set(); for (const z of this.zones)
        for (const k of z.floor || [])
            floor.add(k); for (let y = room.y; y < room.y + room.h; y++)
        for (let x = room.x; x < room.x + room.w; x++)
            for (const [dx, dy] of DIR4) {
                const nx = x + dx, ny = y + dy;
                if (nx >= room.x && ny >= room.y && nx < room.x + room.w && ny < room.y + room.h)
                    continue;
                if (floor.has(this._key(nx, ny)))
                    n++;
            } return n; }
    _noUnintendedZoneConnections() { const zoneAt = Array.from({ length: this.height }, () => Array(this.width).fill(null)); for (const z of this.zones)
        for (let y = z.rect.y; y < z.rect.y + z.rect.h; y++)
            for (let x = z.rect.x; x < z.rect.x + z.rect.w; x++)
                zoneAt[y][x] = z.id; const allowed = new Set(); for (const g of this.gates) {
    ...
    } const floor = new Set(); for (const z of this.zones)
        for (const k of z.floor || [])
            floor.add(k); for (const k of floor) {
        const [x, y] = this._xy(k), z = zoneAt[y][x];
        for (const [dx, dy] of [[1, 0], [0, 1]]) {
            ...
            if (z && z2 && z !== z2 && floor.has(nk) && !allowed.has(k + '>' + nk))
                return false;
        }
    } return true; }
    _noBadRoomTouches() { const roomAt = this._buildRoomAt(this.rooms), floor = new Set(); for (const z of this.zones)
        for (const k of z.floor || [])
            floor.add(k); for (const k of floor) {
        const [x, y] = this._xy(k);
        if (roomAt[y][x])
        ...
        for (const [r, t] of touched) {
            if (!r.doorOutside.has(k))
                return false;
            if (!t.orth)
                return false;
        }
    } return true; }
    runTests() { const tests = [], add = (name, ok, details = '') => tests.push({ name, ok, details }); add('ZoneGraph is connected', this._zoneGraphConnected()); add('Requested special zones were created', this.zones.filter(z => z.type === 'special').length === this.specialCount, 'requested=' + this.specialCount + ', created=' + this.zones.filter(z => z.type === 'special').length); add('Every ZoneGraph edge has at least one gate', this.edges.every(e => (e.gates || []).length >= 1)); add('Special edges have exactly one gate', this.edges.filter(e => this._zone(e.a).type === 'special' || this._zone(e.b).type === 'special').every(e => (e.gates || []).length === 1)); add('Special zones have valid required-neighbor counts', this.zones.filter(z => z.type === 'special').every(z => z.slotClass ? (z.slotClass === 'corner' ? (z.gates || []).length === 2 : (z.gates || []).length === 3) : (z.gates || []).length >= 1), this.zones.filter(z => z.type === 'special').map(z => (z.slot || z.id) + '=' + ((z.gates || []).length)).join(', ')); add('Every zone is internally connected', this.zones.every(z => this._zoneTerminalsConnected(z, z.rooms || [], z.floor || new Set(), false))); add('Every special-zone backbone connects its gates without entering the special room', this.zones.filter(z => z.type === 'special').every(z => this._zoneTerminalsConnected(z, z.rooms || [], z.floor || new Set(), true))); const entrances = this.rooms.filter(r => r.special).map(r => ({ r, n: this._specialEntranceCount(r) })); add('Every special room has exactly one physical entrance', entrances.every(x => x.n === 1), entrances.map(x => x.r.roomType + '@' + x.r.zoneId + '=' + x.n).join(', ')); add('All normal rooms remain globally reachable with special rooms blocked', this._globalReachableWithoutSpecial()); add('No unintended physical connection exists between zones', this._noUnintendedZoneConnections()); add('Room/corridor contacts are valid (corner-side doorways allowed, diagonal-only touch forbidden)', this._noBadRoomTouches()); add('Every alcove/niche is a global single-attachment leaf and does not intersect rooms', this._validateAlcoves(), 'created=' + this.alcoves.length + ', cross-zone=' + (this.stats.alcoveCrossZone || 0) + ', special-area=' + (this.stats.alcoveSpecialArea || 0)); return tests; }
    ...
    generate(layout) {
        if (!layout)
            throw new Error('DungeonGenerator.generate(layout) requires a resolved ZoneLayout');
        const result = this._generateOnce(layout);
        const failedTests = result.tests.filter(t => !t.ok);
        if (failedTests.length)
            throw new Error('Validation failed: ' + failedTests.map(t => t.name).join(', '));
        return result;
    }
    metrics(result) { let floor = 0, roomCells = 0; const roomSet = new Set(); for (const r of result.rooms)
        for (let y = r.y; y < r.y + r.h; y++)
            for (let x = r.x; x < r.x + r.w; x++)
                roomSet.add(this._key(x, y)); for (const a of result.alcoves || [])
        for (let y = a.rect.y; y < a.rect.y + a.rect.h; y++)
            for (let x = a.rect.x; x < a.rect.x + a.rect.w; x++)
                roomSet.add(this._key(x, y)); for (let y = 0; y < this.height; y++)
        for (let x = 0; x < this.width; x++)
            if (result.map.kind[y][x] !== '#')
                floor++; roomCells = roomSet.size; return { floor, roomCells, corridorCells: Math.max(0, floor - roomCells), fillPct: 100 * floor / (this.width * this.height) }; }
    toASCII(result) { return result.map.kind.map(r => r.join('')).join('\n'); }
    static runBatch(iterations, sizes, options = {}) { const s = { maps: 0, passed: 0, failed: 0, exceptions: 0, rooms: 0, special: 0, retries: 0, layoutRetries: 0, loops: 0, extraGates: 0, fallbacks: 0, fallbackMaps: 0, l: 0, astar: 0, routeFailed: 0, loopCandidates: 0, loopsRequested: 0, extraGateCandidates: 0, alcoveRooms: 0, alcoveNiches: 0, alcoveRoomCandidates: 0, alcoveNicheCandidates: 0, failTests: {}, bySize: {}, specialSlots: { nw: 0, n: 0, ne: 0, w: 0, e: 0, sw: 0, s: 0, se: 0 }, firstFailure: null }; for (let i = 0; i < iterations; i++) {
        const size = sizes[i % sizes.length], seed = (options.seedBase || 1) + i, key = size[0] + 'x' + size[1], bs = s.bySize[key] || (s.bySize[key] = { maps: 0, failed: 0, rooms: 0, special: 0, retries: 0, fallbackMaps: 0, minRooms: Infinity, maxRooms: 0 });
        try {
            ...
            for (const z of r.zones.filter(z => z.type === 'special'))
                s.specialSlots[z.slot || z.corner]++;
            if (!failed.length)
                s.passed++;
            else {
                s.failed++;
                ...
                for (const t of failed)
                    s.failTests[t.name] = (s.failTests[t.name] || 0) + 1;
                if (!s.firstFailure)
...
class ZoneGraphMapGenerator {
    constructor(cfg = {}) {
        ...
        this.cfg = this._scenarioSpec ? { ...cfg, ...specGeneration, seed: cfg.seed ?? specGeneration.seed ?? 1, plannerMode: this.plannerMode, zoneGraphSpec: this._scenarioSpec } : { ...cfg };
    ...
    _roomTarget(zone) {
        if (this._lastEngine)
            return this._lastEngine._roomTarget(zone);
        return new DungeonGenerator(this.cfg)._roomTarget(zone);
    }
    metrics(result) {
        if (!this._lastEngine)
            throw new Error('Generate a map before requesting metrics');
        return this._lastEngine.metrics(result);
    }
    toASCII(result) {
        if (!this._lastEngine)
            return result.map.kind.map(r => r.join('')).join('\n');
